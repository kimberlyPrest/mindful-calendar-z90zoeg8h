      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <div className="flex-1 overflow-y-auto p-4 lg:p-8 pt-6">
          <div className="max-w-[1400px] mx-auto h-full flex flex-col">
            ...
            {/* Grid */}
            <div className="grid grid-cols-7 gap-4 min-h-[600px] auto-rows-fr flex-1">
              {days.map((day) => {
                ...
                return (
                  <div
                    key={day.toISOString()}
                    className={cn(
                      "date-cell rounded-2xl p-2 flex flex-col gap-2 min-h-[120px] shadow-sm border border-transparent transition-all relative group cursor-pointer",
                      isOutside ? "bg-slate-50/50 dark:bg-white/5 opacity-40" : "bg-white dark:bg-[#1a2c2e] hover:border-primary/20",
                      isTodayDay && "border-2 border-primary/50 shadow-md bg-primary/[0.02]"
                    )}
      ...
      <aside className="w-80 lg:w-96 bg-white dark:bg-[#1a2c2e] border-l border-slate-100 dark:border-slate-800 flex flex-col h-full overflow-y-auto p-8 shadow-card z-10 flex-shrink-0">
        <div className="bg-gradient-to-br from-primary/10 to-primary/5 dark:from-white/10 dark:to-white/5 rounded-3xl p-6 mb-8 text-center relative overflow-hidden group">
          ...
          <div className="relative z-10">
            <span className="inline-block px-3 py-1 bg-white/60 dark:bg-black/20 rounded-full text-[10px] font-bold tracking-wider uppercase text-primary mb-4 backdrop-blur-sm leading-none">Momento Zen</span>
            <h2 className="text-2xl font-bold text-slate-800 dark:text-white mb-2">Hora de respirar?</h2>
            <p className="text-slate-500 dark:text-slate-300 text-sm mb-6 leading-relaxed">Tire 5 minutos para resetar sua mente antes da próxima tarefa.</p>
            <div className="w-full h-32 mb-6 rounded-2xl bg-white dark:bg-slate-800 overflow-hidden flex items-center justify-center relative shadow-inner">
              <img
                alt="Meditação"
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-col gap-4 relative">
            ...
            {todayEvents.length > 0 ? (
                ...
                <div key={event.id} className={cn("flex gap-4 relative", index !== 0 && "opacity-60 hover:opacity-100 transition-opacity")}>
                  ...
                  <div className={cn(
                    ...
                    <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400 text-xs">
                      <span className="material-icons-round text-sm leading-none">schedule</span>
              ...
              <div className="text-center py-8 text-slate-400 italic text-sm">Sem eventos hoje.</div>
            )}
          </div>
        </div>

        <div className="mt-auto bg-[#fcf5c7]/30 dark:bg-yellow-900/10 p-5 rounded-2xl border border-yellow-100 dark:border-yellow-800/30 shadow-sm">
          <span className="material-icons-round text-yellow-600/50 dark:text-yellow-500/50 text-3xl mb-2 block leading-none">format_quote</span>
          <p className="text-sm font-medium text-slate-700 dark:text-slate-300 italic mb-2 leading-relaxed">"O momento presente é cheio de alegria e felicidade. Se você estiver atento, verá."</p>
